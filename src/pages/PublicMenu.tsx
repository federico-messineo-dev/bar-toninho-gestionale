    ...
    <div className="min-h-screen bg-[#1B1512] font-sans pb-8">
      ...
      <div className="max-w-2xl mx-auto px-4 pt-6 space-y-8">
        {Object.entries(grouped).map(([category, items]: [string, typeof products]) => (
          <section key={category} className="bg-card rounded-3xl p-5 shadow-sm border border-outline-variant">
            <h2 className="font-headline-md text-headline-md text-primary mb-4 border-b border-outline-variant pb-2">{category}</h2>
            <div className="space-y-3">
              {items.map((item) => (
                <div
                  key={item.id}
                  className="flex justify-between items-center py-2 border-b border-outline-variant/50 last:border-0 transition-colors hover:pl-2"
                >
                  <span className="font-body-lg text-on-surface pr-4">{item.name}</span>
                  <span className="font-label-lg text-primary font-semibold shrink-0">€{Number(item.price || 0).toFixed(2)}</span>
                </div>
              ))}
            </div>
          </section>
