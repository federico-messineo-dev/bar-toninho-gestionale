...
const LoginPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#32271F] via-[#1B1512] to-[#120D0A] p-4 relative overflow-hidden">
      ...
      <div className="bg-card rounded-3xl p-8 md:p-10 w-full max-w-md shadow-2xl relative z-10 border border-[#F3E9D7]/15 animate-[fadeIn_0.4s_ease]">
        <div className="text-center mb-8">
          <img src="/toninho-logo.png" alt="Caffè Toninho" className="h-20 w-auto mx-auto mb-3" />
          <p className="font-body-md text-outline">Gestione Magazzino e Menu</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          {error && (
            <div className="bg-error-container text-on-error-container p-3 rounded-full font-label-sm animate-[slideDown_0.2s_ease]">
              {error}
            </div>
          )}
          ...
          <button
            ...
            className="w-full bg-primary text-on-primary font-label-lg py-3.5 rounded-full shadow-md hover:bg-primary/90 transition-colors disabled:opacity-70 cursor-pointer active:scale-[0.98]"
          >
            {loading || authLoading ? (
